"use strict";
var __defProp = Object.defineProperty;
var __getOwnPropDesc = Object.getOwnPropertyDescriptor;
var __getOwnPropNames = Object.getOwnPropertyNames;
var __hasOwnProp = Object.prototype.hasOwnProperty;
var __export = (target, all) => {
  for (var name in all)
    __defProp(target, name, { get: all[name], enumerable: true });
};
var __copyProps = (to, from, except, desc) => {
  if (from && typeof from === "object" || typeof from === "function") {
    for (let key of __getOwnPropNames(from))
      if (!__hasOwnProp.call(to, key) && key !== except)
        __defProp(to, key, { get: () => from[key], enumerable: !(desc = __getOwnPropDesc(from, key)) || desc.enumerable });
  }
  return to;
};
var __toCommonJS = (mod) => __copyProps(__defProp({}, "__esModule", { value: true }), mod);

// src/shared/middlewares/ZodValidation.ts
var ZodValidation_exports = {};
__export(ZodValidation_exports, {
  ZODVerifyDataMiddleware: () => ZODVerifyDataMiddleware
});
module.exports = __toCommonJS(ZodValidation_exports);

// src/shared/Util/AppError/AppError.ts
var AppError = class {
  constructor(message, statusCode = 400) {
    this.message = message;
    this.statusCode = statusCode;
  }
};

// src/shared/middlewares/ZodValidation.ts
var ZODVerifyDataMiddleware = ({ schema }) => {
  const ZodMiddleware = async (request, response, next) => {
    try {
      const data = request.body;
      const dataQuery = request.query;
      const dataFile = request.file;
      const resultParse = schema.parse({ ...data, ...dataQuery, ...dataFile });
      const returnedTarget = Object.assign(data, resultParse);
      request.body = returnedTarget;
      return next();
    } catch (error) {
      const { message: messageError } = JSON.parse(error.message)[0];
      throw new AppError(messageError, 422);
    }
  };
  return ZodMiddleware;
};
// Annotate the CommonJS export names for ESM import in node:
0 && (module.exports = {
  ZODVerifyDataMiddleware
});

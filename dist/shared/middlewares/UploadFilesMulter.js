...
var expectNumber, MAX_FLOAT, expectFloat32, expectLong, expectInt32, expectShort, expectByte, expectSizedInt, castInt, expectNonNull, expectObject, expectString, strictParseFloat32, NUMBER_REGEX, parseNumber, strictParseInt32, strictParseShort, strictParseByte, stackTraceWarning, logger2;
...
var F, G, H, I, J, a3, b3, c3, d3, e3, f3, g3, h3, i3, j3, k3, l3, m3, n3, o3, p3, q3, r3, s3, t3, u3, v3, w3, x3, y, z2, A, B, C, D, E, _data3, ruleSet3;
...
// src/shared/middlewares/UploadFilesMulter.ts
var UploadFilesMulter_exports = {};
__export(UploadFilesMulter_exports, {
  UploadFilesMulter: () => UploadFilesMulter
});
module.exports = __toCommonJS(UploadFilesMulter_exports);

// src/config/configMulter/MulterConfig.ts
...
var at = { [co]: [{ [cp]: true, [i4]: y2, [cq]: z3, [ct]: ["*"] }, { [cp]: true, [i4]: "sigv4", [cq]: z3, [cr]: "{Region}" }] };
...
var bi = { [co]: [{ [cp]: true, [i4]: y2, [cq]: z3, [ct]: ["*"] }, { [cp]: true, [i4]: "sigv4", [cq]: z3, [cr]: "{bucketArn#region}" }] };
var bj = { [ch]: E2, [ci]: [ac] };
...
// src/config/configMulter/Local/AWS_S3.ts
...
var handleCreateHashImg = (originalName) => {
  const createHash3 = import_crypto3.default.randomBytes(16).toString("hex");
  const fileName = `${createHash3}-${originalName}`;
  return fileName;
};
...
var listBuckets = async () => {
  try {
    ...
    logger.fatal("N\xE3o foi poss\xEDvel listar os buckets do S3. Verifique suas credenciais e a rede.", err);
  }
};
listBuckets();

// src/config/configMulter/Local/AWS_S3.ts
var { AWS_NAME_BUCKET } = env;
var MulterS3Config = (0, import_multer_s3.default)({
  s3: ConfigS3,
...
var pathsLocalFiles = import_path6.default.resolve(__dirname, "..", "..", "..", "..", "tmp", "uploads");
var MulterLocalConfig = import_multer.default.diskStorage({
  destination: (req, file, cb2) => cb2(null, pathsLocalFiles),
...
var handleGetLocalStorage = () => {
  ...
  switch (localUpload) {
    ...
    default: {
      throw new AppError("Nenhum m\xE9todo encontrado para fazer upload, entrar em contato com o suporte");
    }
  }
};
var handleGetMulterConfig = ({ fileSize, allowedMimes }) => {
  const multerConfig = {
    ...
    fileFilter: (req, file, cb2) => {
      if (allowedMimes.includes(file.mimetype)) {
        ...
        cb2(new AppError("Tipo de arquivo invalido!"));
      }
    }
  };
  return multerConfig;
...
// src/shared/features/getFileSize.ts
var getFileSize = (bytes) => {
  const k5 = 1024;
  const dm = 2;
  const sizes = ["Bytes", "KB", "MB", "GB", "TB", "PB", "EB", "ZB", "YB"];
  const i5 = Math.floor(Math.log(bytes) / Math.log(k5));
  const result = `${parseFloat((bytes / k5 ** i5).toFixed(dm))} ${sizes[i5]}`;
  return result;
};

// src/shared/middlewares/UploadFilesMulter.ts
var UploadFilesMulter = ({ nameField, sizeLimite, type_image, allowedMimes }) => {
  const UploadFiles = async (req, res, next) => {
    const upload = type_image === "multiple" ? (0, import_multer2.default)(handleGetMulterConfig({ fileSize: sizeLimite, allowedMimes })).array(nameField) : (0, import_multer2.default)(handleGetMulterConfig({ fileSize: sizeLimite, allowedMimes })).single(nameField);
    upload(req, res, (err) => {
      if (err instanceof import_multer2.MulterError) {
        if (err.message === "File too large") {
          return res.status(400).json({ message: `Algum arquivo \xE9 maior que ${getFileSize(sizeLimite)} !` });
        }
        if (err.message === "Unexpected field") {
          return res.status(400).json({ message: `Para fazer upload o campo esperado \xE9 '${nameField}' !` });
        }
      }
      if (err) {
